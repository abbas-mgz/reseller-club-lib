...
const checkDomainStatus = async (domainName) => {
  try {
    const response = await axios.get(`https://api.resellerclub.com/v1/domain-status?domain=${domainName}&apiKey=${process.env.API_KEY}`);
    return response.data;
  } catch (error) {
...
const listDomains = async () => {
  try {
    const response = await axios.get(`https://api.resellerclub.com/v1/domain-list?apiKey=${process.env.API_KEY}`);
    return response.data;
  } catch (error) {
